<template>
  <div class="register_box">
    ...
    <div class="login_inp">
      ...
      <div class="loginBox">
        <label>密码</label>
        <input type="password" id="password" ref="password" v-model="password" placeholder="请设置六位数以上的密码">
      ...
      <div class="login_an" @click="subRegister" :class="{'subBox_color':subBacks}">提交</div>
    </div>
  </div>
</template>
...
//      倒计时
    ...
    subRegister(){
      ...
      if (!tel) {
        ...
        //提交注册
        let fromkey;
        let sessionFromKey = sessionStorage.getItem('fromKey')
        if(!!sessionFromKey && sessionFromKey!='' && sessionFromKey!='false'){
          fromkey = sessionFromKey
        }else {
          fromkey = 'web'
        }
        let regisObj;
        ...
        let _this = this;
        user.register(regisObj).then(function (responce) {
          if (responce.data.code == "0") {
            //如果注册成功，请求登录接口
